...
test('format: reply message labels the sender side and escapes the text', () => {
  const fromStaff = formatTicketReplyMessage({
    ...
    senderFullName: 'Siti', senderUsername: 'siti', senderRole: 'IT Staff', createdAt: '2026-09-24T11:00:00.000Z',
  }, 'https://desk.example');
  expect(fromStaff).toContain('💬 <b>BALASAN BARU — DARI TIM IT</b>');
  expect(fromStaff).toContain('💬 Sudah saya cek &lt;hari ini&gt;.');

  const fromUser = formatTicketReplyMessage({
    ticketNumber: 'TKT-000123', title: 'Printer rusak', messageText: 'Masih error.',
    senderFullName: 'Budi', senderUsername: 'budi', senderRole: 'User', createdAt: '2026-09-24T11:01:00.000Z',
  });
  expect(fromUser).toContain('💬 <b>BALASAN BARU — DARI PELAPOR</b>');
  expect(fromUser).not.toContain('🔗'); // no APP_URL configured => no link line
});
